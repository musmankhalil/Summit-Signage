...
export function updateApp(tokenFromStorage,id,updateData) {
  console.log("---[APP ACTION] updating app", id);
  const request = axios({
    method: 'put',
    data: updateData,
    url: `${ROOT_URL}/app/${id}`,
...
export function updateAppSuccess(activeApp) {
  return {
...
export function updateTimer(tokenFromStorage,props) {
  const request = axios({
    method: 'put',
    data: props,
    url: `${ROOT_URL}/timers`,
